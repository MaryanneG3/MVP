...
import { ShoppingCart, MapPin, Search, Filter, Grid, List, Star, Clock, AlertCircle, CheckCircle } from 'lucide-react';
import { SelectedStore, Product, Store } from './types';
import { productCategories, mockStores, mockProducts, mockPrices } from './data/mockData';
...
function App() {
  ...
  const [storesWithClosestLocations, setStoresWithClosestLocations] = useState<Store[]>([]);
  ...
  const handleStoreRemove = (storeId: string, locationId: string) => {
    setSelectedStores(selectedStores.filter(
  ...
  const handleProductSelect = (product: Product) => {
    ...
    if (selectedStores.length === 0) {
      ...
      productPrices.forEach(price => {
        const store = storesWithClosestLocations.find(s => s.id === price.storeId);
        const location = store?.locations.find(l => l.id === price.locationId);
        
        if (store && location) {
          // Check if this store is already added
          const alreadyAdded = storesWithProduct.some(s => 
            s.store.id === store.id && s.location.id === location.id
          );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {currentView === 'home' && (
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
          ...
          <div className="bg-gradient-to-r from-green-600 to-green-800 rounded-xl lg:rounded-2xl text-white p-6 lg:p-8 mb-6 lg:mb-8">
            <div className="max-w-4xl">
              ...
              <p className="text-lg lg:text-xl text-green-100 mb-4 lg:mb-6">
                Get live prices on real products from major trade stores. Live Prices, Smarter Buys, Bigger Savings!
              </p>
              {userLocation && (
                <div className="flex items-center space-x-2 mb-4 lg:mb-6 text-green-100">
                  <MapPin className="h-4 w-4" />
                  <span className="text-sm lg:text-base">Showing closest stores near {userLocation}</span>
                </div>
              )}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 lg:p-6 mb-6 lg:mb-8">
            <h2 className="text-lg lg:text-xl font-semibold text-gray-900 mb-3 lg:mb-4">Product Search</h2>
            <div className="flex flex-col sm:flex-row gap-3 lg:gap-4">
          ...
          <div className="mb-6 lg:mb-8">
            <h2 className="text-xl lg:text-2xl font-bold text-gray-900 mb-4 lg:mb-6">Product Categories</h2>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 lg:p-6">
            <h2 className="text-lg lg:text-xl font-semibold text-gray-900 mb-3 lg:mb-4">Our Store Network</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 lg:gap-4">
              {storesWithClosestLocations.map(store => (
                <div key={store.id} className="text-center p-2 lg:p-3 rounded-lg hover:bg-green-50 transition-colors">
                  <div className="text-xl lg:text-2xl mb-1 lg:mb-2">{store.logo}</div>
                  <h3 className="text-xs lg:text-sm font-medium text-gray-900">{store.name}</h3>
      ...
      {currentView === 'stores' && (
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
          <div className="mb-4 lg:mb-6">
            <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 mb-2">Select Stores to Compare</h1>
            <p className="text-gray-600 text-sm lg:text-base">
              Choose up to 3 trade stores from the closest locations near {userLocation}
            </p>
          </div>
      ...
      {currentView === 'products' && (
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
          <div className="mb-4 lg:mb-6">
            <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 mb-2">Browse Products</h1>
            <p className="text-gray-600 text-sm lg:text-base">
              {selectedStores.length > 0 
