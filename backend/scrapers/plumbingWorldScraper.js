// Specialized scraper for Plumbing World stores
const puppeteer = require('puppeteer');

class PlumbingWorldScraper {
  constructor() {
    this.baseUrl = 'https://www.plumbingworld.co.nz';
    this.storeLocatorUrl = `${this.baseUrl}/store-locator`;
  }

  async scrapeAllStores() {
    console.log('Starting Plumbing World store scraping...');
    ...
    try {
      ...
      console.log(`Found ${stores.length} Plumbing World stores`);
      return stores;
      
    } catch (error) {
      console.error('Error scraping Plumbing World stores:', error);
      return [];
    } finally {
      await browser.close();
    }
  ...
  async extractStoreData(page) {
    ...
    try {
      ...
      console.error('Error extracting Plumbing World store data:', error);
    }
  ...
  formatStoreData(rawStores, source) {
    return rawStores.map(store => ({
      id: `plumbing-world-${this.generateStoreId(store)}`,
      chain: 'plumbing-world',
      name: 'Plumbing World',
...
module.exports = PlumbingWorldScraper;