// Specialized scraper for Repco stores
const puppeteer = require('puppeteer');

class RepcoScraper {
  constructor() {
    this.baseUrl = 'https://www.repco.co.nz';
    this.storeLocatorUrl = `${this.baseUrl}/stores`;
  }

  async scrapeAllStores() {
    console.log('Starting Repco store scraping...');
    ...
    try {
      ...
      console.log(`Found ${stores.length} Repco stores`);
      return stores;
      
    } catch (error) {
      console.error('Error scraping Repco stores:', error);
      return [];
    } finally {
      await browser.close();
    }
  ...
  async extractStoreData(page) {
    ...
    try {
      ...
      console.error('Error extracting Repco store data:', error);
    }
  ...
  formatStoreData(rawStores, source) {
    return rawStores.map(store => ({
      id: `repco-${this.generateStoreId(store)}`,
      chain: 'repco',
      name: 'Repco',
